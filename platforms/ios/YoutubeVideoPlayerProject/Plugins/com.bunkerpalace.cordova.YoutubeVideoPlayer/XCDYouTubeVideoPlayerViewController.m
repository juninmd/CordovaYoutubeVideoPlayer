...
NSString *const XCDMoviePlayerPlaybackDidFinishErrorUserInfoKey = @"error";
...
- (void) setVideoIdentifier:(NSString *)videoIdentifier
{
	...
	self.videoOperation = [[XCDYouTubeClient defaultClient] getVideoWithIdentifier:videoIdentifier completionHandler:^(XCDYouTubeVideo *video, NSError *error) {
		[self handleVideoResponseWithVideo:video error:error];
	}];
}
...
- (void) handleVideoResponseWithVideo:(XCDYouTubeVideo *)video error:(NSError *)error
{
	if (!video) {
