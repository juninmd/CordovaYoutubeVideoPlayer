...
- (void) startNextVideoInfoRequest
{
	...
	NSString *eventLabel = self.eventLabels[0];
	...
	[self startRequestWithURL:[NSURL URLWithString:[@"https://www.youtube.com/get_video_info?" stringByAppendingString:XCDQueryStringWithDictionary(query, NSUTF8StringEncoding)]]];
}
...
- (void) handleVideoInfoResponseWithInfo:(NSDictionary *)info playerScript:(XCDYouTubePlayerScript *)playerScript
{
	...
	if ([error.domain isEqual:XCDYouTubeVideoErrorDomain] && error.code == XCDYouTubeErrorUseCipherSignature) {
		...
		[self startRequestWithURL:[NSURL URLWithString:[@"https://www.youtube.com/watch?" stringByAppendingString:XCDQueryStringWithDictionary(query, NSUTF8StringEncoding)]]];
		return;
	}

	self.lastError = error;
	if (error.code > 0) self.youTubeError = error;
...
- (void) finishWithVideo:(XCDYouTubeVideo *)video { self.video = video; [self finish]; }

- (void) finishWithError:(NSError *)error { self.error = error; [self finish]; }
...
- (void) cancel { [super cancel]; [self.connection cancel]; [self finish]; }

- (void) finish { self.isExecuting = NO; self.isFinished = YES; }
...
- (void) connection:(NSURLConnection *)connection didReceiveData:(NSData *)data { [self.connectionData appendData:data]; }

- (void) connectionDidFinishLoading:(NSURLConnection *)connection
{
	NSString *path = connection.originalRequest.URL.path;

	if ([path isEqualToString:@"/get_video_info"]) {
		NSString *videoQuery = [[NSString alloc] initWithData:self.connectionData encoding:NSASCIIStringEncoding];
		[self handleVideoInfoResponseWithInfo:XCDDictionaryWithQueryString(videoQuery, NSUTF8StringEncoding) playerScript:nil];
	} else if ([path isEqualToString:@"/watch"]) {
...
{
	self.lastError = [NSError errorWithDomain:XCDYouTubeVideoErrorDomain code:XCDYouTubeErrorNetwork userInfo:@{
		NSLocalizedDescriptionKey: connectionError.localizedDescription,
